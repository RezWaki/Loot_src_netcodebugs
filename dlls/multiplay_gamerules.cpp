...
CHalfLifeMultiplay :: CHalfLifeMultiplay()
{
	...
	/*if( !CVAR_GET_FLOAT("mp_teamplay") || !g_teamplay ) {
		if( IS_DEDICATED_SERVER ) return;
		//if( !IS_DEDICATED_SERVER ) {
			MessageBoxA(NULL, "Couldn't start server in DM mode!", "Error", MB_ICONERROR);
			Sleep( -1 );
		//}
	}*/
}
...
char msg_c[256];
hudtextparms_t tprms_c;
std::string weaponname_sub_c;
...
{
	tprms_c.channel = -1;
	tprms_c.r1 = tprms_c.r2 = 255;
	tprms_c.g1 = tprms_c.g2 = tprms_c.b1 = tprms_c.b2 = 0;
	tprms_c.fadeinTime = 1;
	tprms_c.fadeoutTime = 1;
	tprms_c.holdTime = 3;
	tprms_c.x = 0;
	tprms_c.y = 0;
	weaponname_sub_c = pWeapon->pszName();
	sprintf( msg_c, "Picked up %s", weaponname_sub_c.substr(7, strlen(weaponname_sub_c.c_str())-7).c_str() );
	UTIL_HudMessage( pPlayer, tprms_c, msg_c );
}
