...
DLL_GLOBAL	short	g_sPlasmaIndex; // hi :+)
...
void W_Precache(void)
{
	...
	//subgren
	UTIL_PrecacheOtherWeapon( "weapon_subgren" );
	...
	g_sPlasmaIndex = PRECACHE_MODEL( "sprites/animglow01.spr"); //plasma shot
	...
	for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		if ( m_rgpPlayerItems[ i ] )
		{
			...
			while ( m_rgpPlayerItems[ i ] )
			{
				...
				{
					if( pPlayer->bHaveRifle && pItem->pszName() == "weapon_9mmAR"
						|| pPlayer->bHaveRifle && pItem->pszName() == "weapon_shotgun"
							|| pPlayer->bHaveRifle && pItem->pszName() == "weapon_crossbow") {
								//bDisallowPickup = TRUE;
								return;
					}
					//else{
						//bDisallowPickup = FALSE;
