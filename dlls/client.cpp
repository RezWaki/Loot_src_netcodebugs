...
// turn on color set 2  (color on,  no sound)
	if ( teamonly )
		...
		sprintf( text, "%c%s says: ", 2, STRING( pEntity->v.netname ) );
	...
	player->m_flNextChatTime = 0; //spam fully allowed
...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq( "spectate", CMD_ARGV(NULL) ) )	// added for proxy support
	{
		CBasePlayer * pPlayer = GetClassPtr((CBasePlayer *)pev);

		while( !pPlayer->pSpecTarget ) {
			iPlrId++;
			pPlayer->pSpecTarget = (CBasePlayer*)UTIL_PlayerByIndex( iPlrId );
		}
		pPlayer->bSpectator = TRUE; //we are spectator NOW
		//edict_t *pentSpawnSpot = g_pGameRules->GetPlayerSpawnSpot( pPlayer );
		pPlayer->StartObserver( pPlayer->pSpecTarget->pev->origin, pPlayer->pSpecTarget->pev->angles);
		pPlayer->pev->flags = FL_SPECTATOR;
		pPlayer->pev->movetype = MOVETYPE_NOCLIP;
		pPlayer->RemoveAllItems(TRUE);
		pPlayer->pev->model = NULL;
	}
	...
	else
	{
		...
		// tell the user they entered an unknown command
		ClientPrint( &pEntity->v, HUD_PRINTCONSOLE, UTIL_VarArgs( "ERROR @ %s\n", command ) );
	}
}
...
void ClientUserInfoChanged( edict_t *pEntity, char *infobuffer )
{
	...
	if ( pEntity->v.netname && STRING(pEntity->v.netname)[0] != 0 && !FStrEq( STRING(pEntity->v.netname), g_engfuncs.pfnInfoKeyValue( infobuffer, "name" )) )
	{
		...
		char text[256];
		sprintf( text, "* %s is now known also %s\n", STRING(pEntity->v.netname), g_engfuncs.pfnInfoKeyValue( infobuffer, "name" ) );
		MESSAGE_BEGIN( MSG_ALL, gmsgSayText, NULL );
			WRITE_BYTE( ENTINDEX(pEntity) );
			WRITE_STRING( text );
		MESSAGE_END();
		...
		if ( g_teamplay )
		{
			UTIL_LogPrintf( "\"%s<%i><%s><%s>\" is now known also \"%s\"\n", 
				STRING( pEntity->v.netname ), 
				GETPLAYERUSERID( pEntity ), 
				GETPLAYERAUTHID( pEntity ),
		...
		{
			UTIL_LogPrintf( "\"%s<%i><%s><%i>\" is now known also \"%s\"\n", 
				STRING( pEntity->v.netname ), 
				GETPLAYERUSERID( pEntity ), 
				GETPLAYERAUTHID( pEntity ),
				GETPLAYERUSERID( pEntity ), 
