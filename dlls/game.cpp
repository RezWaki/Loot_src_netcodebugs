...
cvar_t	roundlimit	= {"mp_roundlimit","15", FCVAR_SERVER };
...
cvar_t onerifle={"sv_onerifle", "1", FCVAR_SERVER}; //player can have only 1 kind of rifles
cvar_t maxfov={"sv_maxfov", "120", FCVAR_SERVER}; //maxfov, default - 120
cvar_t skipround={"sv_allowskipround", "0", FCVAR_SERVER}; //off by default
cvar_t lootrop={"sv_allowlootdrop", "0", FCVAR_SERVER}; //off by default
...
void GameDLLInit( void )
{
	...
	CVAR_REGISTER(&onerifle);
	CVAR_REGISTER(&roundlimit);
	CVAR_REGISTER(&maxfov);
	CVAR_REGISTER(&skipround);
	CVAR_REGISTER(&lootrop);
