...
int gmsgTeamNames = 0;
INT gmsgSmokePart_xpos = 0; //smk effect x
INT gmsgSmokePart_ypos = 0; //smk effect y
INT gmsgSmokePart_zpos = 0; //smk effect z
INT gmsgViewangle = 0; //if spec
...
void LinkUserMessages( void )
{
	...
	gmsgSmokePart_xpos = REG_USER_MSG( "Smoke_X", -1 );
	gmsgSmokePart_ypos = REG_USER_MSG( "Smoke_Y", -1 );
	gmsgSmokePart_zpos = REG_USER_MSG( "Smoke_Z", -1 );
...
char msg[512]; //console
hudtextparms_t tprms;
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	//pev->deadflag = DEAD_DEAD;
	UTIL_ScreenFade( this, Vector(128,0,0), 6, 120, 255, FFADE_OUT | FFADE_MODULATE );

	//this->bDead = TRUE;
	this->bHaveRifle = FALSE; //remove rifle when dead

	SetAnimation( PLAYER_DIE );

	UTIL_SayText("You will respawn in next round!", this);

	if( bHaveLootBox ) {
		CItemLootBox* pLootBox = (CItemLootBox*)CItem::Create( "item_lootbox", pev->origin + gpGlobals->v_forward * 50, Vector(0, 0, 0), edict() );
		pLootBox->pev->velocity = gpGlobals->v_forward * 300 + gpGlobals->v_forward * 100;
		pLootBox->bDropped = TRUE;
		bHaveLootBox = FALSE;
		pev->renderfx = kRenderFxNone;
		for(int iCurrent = 0; iCurrent <= 32; iCurrent++){
			CBaseEntity* pPlr = UTIL_PlayerByIndex( iCurrent );
			if( pPlr ) {
				if( pPlr->TeamID() == this->TeamID() ) {
					tprms.channel = -1;
					tprms.r1 = tprms.r2 = 255;
					tprms.g1 = tprms.g2 = tprms.b1 = tprms.b2 = 0;
					tprms.fadeinTime = 1;
					tprms.fadeoutTime = 1;
					tprms.holdTime = 3;
					tprms.x = 0.4;
					tprms.y = 0.5;
					//UTIL_SayText("Your team lost LootBox!", pPlr);
					sprintf( msg, "%s has lost LootBox!", STRING(pev->netname) );
					//UTIL_SayText(msg, pPlr);
					UTIL_HudMessage(pPlr, tprms, msg);
				}
			}
		}
	}
	//EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/die.wav", 1, ATTN_NORM);
	/*//Spawn();
	RemoveAllItems(TRUE);
	pev->model = NULL;
	pev->iuser1 = 3;
...
void CBasePlayer::SetAnimation( PLAYER_ANIM playerAnim )
{
	...
	if( pev->flags == FL_SPECTATOR || bSpectator ) {
			tprms.channel = 2;
			tprms.r1 = tprms.r2 = 255;
			tprms.g1 = tprms.g2 = tprms.b1 = tprms.b2 = 0;
			tprms.fadeinTime = 0;
			...
			tprms.x = 0;
			tprms.y = 0;
			sprintf( msg, "You are spectating for %s", STRING(pSpecTarget->pev->netname) );
			UTIL_HudMessage( this, tprms, msg );
			/*if( this->entindex() == pSpecTarget->entindex() ) {
				this->pev->flags = FL_CLIENT;
				this->bSpectator = FALSE;
				pev->movetype = MOVETYPE_WALK;
				g_engfuncs.pfnClientPrintf( this->edict(), PRINT_TYPE::print_console, "No available target!\nExiting spectator mode..." );
				//edict_t *pRespawnSpot = g_pGameRules->GetPlayerSpawnSpot( this );
				//this->pev->origin = pRespawnSpot->v.origin;
				this->Spawn();
				//this->pev->origin = pRespawnSpot->v.origin;
				//this->Respawn();
				return;
			}*/
			pev->origin = Vector( this->pSpecTarget->pev->origin.x+32, this->pSpecTarget->pev->origin.y-16, this->pSpecTarget->pev->origin.z+72 );
			pev->angles = pSpecTarget->pev->angles;
			//g_engfuncs.pfnSetView( pSpecTarget->edict(), this->edict() );
			/*MESSAGE_BEGIN( MSG_ONE, gmsgViewangle, NULL, this->pev);
				WRITE_ANGLE( this->pev->angles.x );
				WRITE_ANGLE( this->pev->angles.y );
				WRITE_ANGLE( this->pev->angles.z );
			MESSAGE_END();*/
	}

	if( FStrEq( "changeteam", CMD_ARGV(NULL) ) ) {
		int length, char_count = 0;
		char *pFileList;
		//char *aFileList = pFileList = /*(char*)LOAD_FILE_FOR_ME( (char *)CVAR_GET_STRING( "motdfile" ), &length )*/"testtext";
		char *aFileList = pFileList = "Select team you want to play\nor maybe spectate?";

		// send the server name
		/*MESSAGE_BEGIN( MSG_ONE, gmsgServerName, NULL, this->edict() );
		...
		while ( pFileList && *pFileList && char_count < 512 )
		{
			char chunk[512+1];
		
			if ( strlen( pFileList ) < 512 )
			{
				strcpy( chunk, pFileList );
			}
			else
			{
				strncpy( chunk, pFileList, 512 );
				chunk[512] = 0;		// strncpy doesn't always append the null terminator
			}
			...
			MESSAGE_BEGIN( MSG_ONE, gmsgMOTD, NULL, this->edict() );
	...
	if( FStrEq( "skipround", CMD_ARGV(NULL) ) && CVAR_GET_FLOAT("sv_allowskipround") ) {
		//CVAR_SET_FLOAT("skipround", 0);
		iRoundNum++;
		for(int iCurrent = 0; iCurrent < 32; iCurrent++){
			CBaseEntity* pPlr = UTIL_PlayerByIndex(iCurrent);
			if( pPlr ) {
	...
	if( this->bHaveLootBox ) {
		pev->rendercolor = Vector(255, 80, 0);
		pev->renderfx = kRenderFxGlowShell;
		//pev->body = 2;
		if( pev->velocity.Length() > 370 ) {
			pev->velocity.x /= 1.5;
			pev->velocity.y /= 1.5;
		}
	}

	if( this->bHaveLootBox && FStrEq( "droploot", CMD_ARGV(NULL) ) && CVAR_GET_FLOAT("sv_allowlootdrop") ) {
		this->bHaveLootBox = FALSE;
		EMIT_SOUND(edict(), CHAN_VOICE, "items/dropped_loot.wav", 1, ATTN_NORM);
		CItemLootBox* pLootBox = (CItemLootBox*)CItem::Create( "item_lootbox", pev->origin + gpGlobals->v_forward * 50, Vector(0, 0, 0), edict() );
		pLootBox->pev->velocity = gpGlobals->v_forward * 300 + gpGlobals->v_forward * 100;
		pLootBox->bDropped = TRUE;
		tprms.channel = -1;
		...
		tprms.x = 0.4;
		tprms.y = 0.5;
		sprintf( msg, "%s has dropped LootBox!", STRING(pev->netname) );
		for(int iCurrent = 0; iCurrent <= 32; iCurrent++){
			CBaseEntity* pPlr = UTIL_PlayerByIndex( iCurrent );
			if( pPlr ) {
				if( pPlr->TeamID() == this->TeamID() ) {
	...
	/*if( this->pev->takedamage != DAMAGE_NO ) {
		UTIL_ScreenFade(this, Vector(255, 0, 0), 1, 0.5, 150, NULL);
	}*/

	/*if( this->pev->fov > CVAR_GET_FLOAT("sv_maxfov") || this->m_iClientFOV > CVAR_GET_FLOAT("sv_maxfov") ) {
		pev->fov = this->m_iFOV = this->m_iClientFOV = CVAR_GET_FLOAT("sv_maxfov");
		g_engfuncs.pfnClientPrintf( this->edict(), PRINT_TYPE::print_console, "FOV is too high. Resetting..." );
	}*/
...
{
	if( this->bFirstSpawn ){
		this->bFirstSpawn = FALSE;
		this->iDeadRound = -1;
	}
	if( this->iDeadRound == iRoundNum ) {
		bCanPickupLoot = FALSE;
		return;
	}
	if( this->bDead ) {
		this->bDead = FALSE; //no more dead
		RemoveAllItems( FALSE ); //remove all wpns if player was dead
	}
	//this->bHaveSilencer = FALSE;
	UTIL_ScreenFade( this, Vector(0, 0, 0), 0, 0, 0, NULL );
	pev->iuser1 = pev->iuser2 = pev->iuser3 = pev->iuser4 = 0;
	bCanPickupLoot = TRUE;
...
// dont let uninitialized value here hurt the player
	...
	this->bHaveLootBox = FALSE; //nobody have lootbox at start
	...
	EMIT_SOUND( this->edict(), CHAN_VOICE, "player/spawn.wav", 1, ATTN_NORM );
...
void CBasePlayer :: Precache( void )
{
	...
	PRECACHE_SOUND("items/dropped_loot.wav");
...
void CBasePlayer::DropPlayerItem ( char *pszItemName )
{
	...
	UTIL_SayTextAll( pszItemName, this );

	//okay this bullshit dont works

	if( pszItemName == "weapon_9mmAR" ) {
		this->bHaveRifle = FALSE; //no more
	}
	else if( pszItemName == "weapon_crossbow" ) {
		this->bHaveRifle = FALSE; //no more
	}
	else if( pszItemName == "weapon_shotgun" ) {
		this->bHaveRifle = FALSE; //no more
	}
	...
	for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		...
		if ( pWeapon )
		{
			...
			//sprintf( msg, "say_team Dropping %s", pWeapon->pszName() ); //server to tried stuff bad command
			//& u suck

			//UTIL_SayText( pWeapon->pszName(), this ); //this works some

			//sprintf( msg, "You've dropped %s", pWeapon->pszName() );
			weaponname_sub = pWeapon->pszName();
			//msg = weaponname_sub.substr(7, strlen(weaponname_sub.c_str())-7);

			EMIT_SOUND( this->edict(), CHAN_ITEM, "items/weapondrop1.wav", 1, ATTN_NORM );

			sprintf( msg, "You've dropped %s", weaponname_sub.substr(7, strlen(weaponname_sub.c_str())-7).c_str() );
			UTIL_SayText( msg, this );

			//this part of code works perfectly :p

			if( pWeapon->pszName() == "weapon_9mmAR" ) {
				this->bHaveRifle = FALSE;
			}
			else if( pWeapon->pszName() == "weapon_crossbow" ) {
				this->bHaveRifle = FALSE;
			}
			else if( pWeapon->pszName() == "weapon_shotgun" ) {
				this->bHaveRifle = FALSE;
			}
			...
			//UTIL_SayText( pWeapon->pszName(), this );
