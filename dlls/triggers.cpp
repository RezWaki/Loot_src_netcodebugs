...
extern INT		gmsgSmokePart_xpos;
extern INT		gmsgSmokePart_ypos;
extern INT		gmsgSmokePart_zpos;
...
//smoke source
/*class CSmokeTrigger : public CBaseTrigger
{
public:
	void Spawn( void );
	void EXPORT SmokeDraw( void );
};

LINK_ENTITY_TO_CLASS( trigger_smokesrc, CSmokeTrigger );

INT iPlrId_v = -1;
CBaseEntity* WORLD;

void CSmokeTrigger::Spawn( void ) {
	pev->nextthink = gpGlobals->time + 0.1;
	//timing
	SetThink( &CSmokeTrigger::SmokeDraw );
	//SmokeDraw();
	InitTrigger();
}

void CSmokeTrigger::SmokeDraw( void ) {
	//iPlrId_v++;
	//if( iPlrId_v >= 32 ) iPlrId_v = -1;
	WORLD = UTIL_PlayerByIndex( 0 );

	//if( !EffectSpectator->IsPlayer() ) return;

	MESSAGE_BEGIN( MSG_ALL, gmsgSmokePart_xpos, NULL );
		WRITE_COORD( this->pev->origin.x );
	MESSAGE_END();

	MESSAGE_BEGIN( MSG_ALL, gmsgSmokePart_ypos, NULL);
		WRITE_COORD( this->pev->origin.y );
	MESSAGE_END();

	MESSAGE_BEGIN( MSG_ALL, gmsgSmokePart_zpos, NULL );
...
//LOOT
class CLootTrigger : public CBaseTrigger
{
public:
	void Spawn( void );
	void LootTouch( CBaseEntity* pOther );
};

LINK_ENTITY_TO_CLASS( trigger_loot, CLootTrigger );


void CLootTrigger::Spawn( void ) {
	InitTrigger();
	SetTouch( &CLootTrigger::LootTouch );
	PRECACHE_SOUND("ambience/win.wav");
}
...
void CLootTrigger::LootTouch( CBaseEntity* pOther ) {
	if( pOther->bHaveLootBox ) {
		pOther->bHaveLootBox = FALSE; //no more lootbox
		pOther->pev->renderfx = kRenderFxNone; //remove renderfx
		CBasePlayer* pPlayer_Entity = (CBasePlayer*)pOther;
		//pPlayer_Entity->AddPointsToTeam( 10, TRUE );
		//g_pGameRules->bCanSpawnMp5 = TRUE;
		for(int iCurrent = -1; iCurrent <= 32; iCurrent++){
			CBaseEntity* pPlr = UTIL_PlayerByIndex(iCurrent);
			if( pPlr ) {
				pPlr->Spawn();
				EMIT_SOUND( pPlr->edict(), CHAN_VOICE, "ambience/win.wav", 1, ATTN_NORM );
			}
		}
		iRoundNum++;
		sprintf(message, "Team %s taken LootBox to base!\nGet ready! Round #%d", pOther->TeamID(), iRoundNum+1);
		if( iRoundNum >= CVAR_GET_FLOAT("mp_roundlimit") ) {
			sprintf(message, "Team %s taken LootBox to base!\nGame finished!", pOther->TeamID());
			g_pGameRules->EndMultiplayerGame();
		}
		UTIL_CenterPrintAll(message, NULL, NULL, NULL, NULL);
		MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
		WRITE_BYTE( pOther->entindex() );
			WRITE_SHORT( pPlayer_Entity->pev->frags+10 );
			WRITE_SHORT( pPlayer_Entity->m_iDeaths );
			WRITE_SHORT( 0 );
			WRITE_SHORT( g_pGameRules->GetTeamIndex( pPlayer_Entity->m_szTeamName ) + 1 );
		MESSAGE_END();
	}
}
