...
public:
	...
	CBaseEntity() { 	
		iDeadRound = -1;
		bHaveSilencer = bHaveRifle = bHaveLootBox = FALSE; 
	};
...
// still realize that they are teammates. (overridden for monsters that form groups)
	...
	virtual BOOL	RemovePlayerItem( CBasePlayerItem *pItem ) { return 0; }
			BOOL	bHaveLootBox;
			INT		iDeadRound; //round when you dead
			BOOL	bCanPickupLoot;
			BOOL	bHaveRifle;
			BOOL	bHavePistol;
			BOOL	bHaveSilencer;
			BOOL	bDead;
			BOOL	bFirstSpawn; //first spawn ever (on map)
			BOOL	bSpectator;
			CBasePlayer* pSpecTarget; //if you is spec guy
