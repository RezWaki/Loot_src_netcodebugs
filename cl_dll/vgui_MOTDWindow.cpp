...
CMessageWindowPanel::CMessageWindowPanel( const char *szMOTD, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall ) : CMenuPanel( iShadeFullscreen ? 100 : 255, iRemoveMe, x, y, wide, tall )
{
	...
	CommandButton *pBlueBtn = new CommandButton( "BLUE", m_pBackgroundPanel->getWide(), iYPos + iYSize - YRES(16) - BUTTON_SIZE_Y, 86, 24);
	pBlueBtn->addActionSignal( new CMenuHandler_StringCommandWatch( "model blue", true ) );
	pBlueBtn->setParent(this);

	CommandButton *pRedBtn = new CommandButton( "RED", iXPos+24, iYPos + iYSize - YRES(16) - BUTTON_SIZE_Y, 86, 24);
	pRedBtn->addActionSignal( new CMenuHandler_StringCommandWatch( "model red", true ) );
	pRedBtn->setParent(this);

	CommandButton *pSpecBtn = new CommandButton( "SPECTATE", (ScreenWidth/2)-24, iYPos + iYSize - YRES(16) - BUTTON_SIZE_Y, 86, 24);
	pSpecBtn->addActionSignal( new CMenuHandler_StringCommandWatch( "spectate", true ) );
	pSpecBtn->setParent(this);
}
