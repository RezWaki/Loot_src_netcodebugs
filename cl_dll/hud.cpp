...
int __MsgFunc_GetViewangles( const char *pszName, int iSize, void *pbuf) {
	BEGIN_READ( pbuf, iSize );
	vPlrViewangles.x = READ_ANGLE();
	vPlrViewangles.y = READ_ANGLE();
	vPlrViewangles.z = READ_ANGLE();
	return 1;
}

/*int __MsgFunc_GetSmokeX( const char *pszName, int iSize, void *pbuf) {
	BEGIN_READ( pbuf, iSize );
	vSmkOrigin.x = READ_COORD();
	return 1;
}
int __MsgFunc_GetSmokeY( const char *pszName, int iSize, void *pbuf) {
	BEGIN_READ( pbuf, iSize );
	vSmkOrigin.y = READ_COORD();
	return 1;
}
int __MsgFunc_GetSmokeZ( const char *pszName, int iSize, void *pbuf) {
	BEGIN_READ( pbuf, iSize );
	vSmkOrigin.z = READ_COORD();
	return 1;
}*/
...
void CHud :: Init( void )
{
	...
	//HOOK_MESSAGE( GetSmokeX );
	//HOOK_MESSAGE( GetSmokeY );
	//HOOK_MESSAGE( GetSmokeZ );
