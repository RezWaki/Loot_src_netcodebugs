...
extern Vector vPlrViewangles;
//#define TRI_API *gEngfuncs.pTriAPI;
...
const char* numvertexes;
qboolean bDefinedColor = false;
int iRadius = 0;
...
void CStudioModelRenderer::StudioRenderFinal_Hardware( void )
{
	...
	//if( vPlrViewangles.x != -1 && vPlrViewangles.y != -1 && vPlrViewangles.z != -1 ) {
		//gEngfuncs.SetViewAngles( vPlrViewangles );
	//}

	//gEngfuncs.CL_LoadModel("sprites/xsmoke1.spr", 0);

	//gEngfuncs.LoadMapSprite("");
	/*particle_t* pSmkPrt;
	pSmkPrt = gEngfuncs.pEfxAPI->R_AllocParticle( ParticleCallback );
	pSmkPrt->color = gEngfuncs.pEfxAPI->R_LookupColor( '150', '150', '150' );
	if( iRadius < 256 ) {
		iRadius++;
		pSmkPrt->org.x = vSmkOrigin.x+iRadius;
		pSmkPrt->org.y = vSmkOrigin.y+iRadius;
		pSmkPrt->org.z = vSmkOrigin.z;
	}
	else{
		iRadius = 0;
	}*/
	//if( iRadius < 64 ) {
	//	iRadius += 4;
		//gEngfuncs.pTriAPI->Vertex3f( vSmkOrigin.x, vSmkOrigin.y, vSmkOrigin.z );
	//}
	//else{
	//	iRadius = 0;
	//}
//	gEngfuncs.pTriAPI->End(); //gg
	//}
	...
	//model_s* map = gEngfuncs.GetEntityByIndex(NULL)->model;

	//mapclr.r = 255; mapclr.g = mapclr.b = 0;
